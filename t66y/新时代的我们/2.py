...
def get_info(site, titles, links):
    ...
    for i in td:
        ...
        titles.append(i.contents[0].replace('\n', '').replace('\r', '').replace('\t', '') + ' ' + i.h3.text.replace('?', ''))
...
def download_img(title, link):
    ...
    if os.path.exists('jpg/' + title):
        return
    os.makedirs('jpg/' + title)
    re = s.get(link, headers=header)
    re.encoding = 'gbk'
    div = BeautifulSoup(re.text, "html.parser").find_all('div', class_='tpc_content do_not_catch')[0]
    img = BeautifulSoup(str(div), "html.parser").find_all('img')
    pbar = tqdm(total=len(img))
    for i in img:
        file_name = 'jpg/' + title + '/' + str(img.index(i)) + '.jpg'
